...
| **Fallback**    | `mistv3` (planned, not present)      |
| **Voice**       | `celeste`                      |
| **Language**     | `en`                          |
...
- Rime TTS then speaks the word slowed down (timeScaleFactor=1.5).
...
> Preflight measured on 2026-09-06. Automated latency run on 2026-09-08 saved to `fixtures/latency_results.json`. Pipeline metrics are tracked per-session by `measure.py` and sent to the client metrics panel.
...
| Assessment latency (sentence 7, period, 2026-09-08) | — | 1024.9ms | — | `fixtures/latency_results.json`, noise input, accuracy 0.0, flagged 8 |
| Assessment latency (sentence 7b, exclamation, 2026-09-08) | — | 729.9ms | — | `fixtures/latency_results.json`, noise input, accuracy 0.0, flagged 8 |
| Rime TTS synthesis (2026-09-08 run) | — | TODO (script error, no audio) | — | `measure_latency.py` failed: Rime plugin used outside LiveKit job context, 6/6 errors, no new mp3s |
| Detection→correction latency | ≤ 1s | TODO | TODO | End of speech → first Rime byte |
| Pronunciation flag accuracy | ≥ 8/10 | TODO | — | Per-word wav2vec2 GOP |
| Interruption stop time | ≤ 300ms | TODO | — | Generation fence + session.interrupt() |
...
| `fixtures/rime_preflight_slow.mp3` | Slowed greeting from preflight (timeScaleFactor=1.5) |
